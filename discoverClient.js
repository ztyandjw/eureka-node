...
var instanceInfo = instanceInfoFactory.create('nodeClient',8866)
var instanceInfoReplicator = require('./instanceInfoReplicator')
var Applications = require('./applications.js')
var localRegionApps = null


funtion fetchRegistry(forceFullRegistryFetch) {
...
function reconcileAndLogDifference(delta, reconcileHashCode) {
...
async function renew(instanceInfo) {
	...
	try {
		...
		console.log('sendHeartbeat!!!! ' + httpResponse.statusCode)
...
async function register(instanceInfo) {
	appPathIdentifier = instanceInfo.getAppName() + '/' + instanceInfo.getId()
	console.log(PREFIX + ' ' + appPathIdentifier + ' : egistering service...')
	try {
...
// instanceInfoReplicator.start(instanceInfo)

// heartBeatLoop(instanceInfo)
